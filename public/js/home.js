...
jQuery(function ($) {
    ...
    $(document).on('click', '.select-time-dropdown, .select-time-dropdown *', function (e) {
        e.stopPropagation();
        updateTimeSearchFields();
    });

    $(document).on('change', '.select-time-dropdown select', function (e) {
        updateTimeSearchFields();
    });

    function updateTimeSearchFields() {
        let parent = $(".form-select-time");
        let from_hour = parent.find('[name=from_hour]').val().toString().trim();
        let to_hour = parent.find('[name=to_hour]').val().toString().trim();
        ...
        let fromHtml = parent.find('.render .from_hour .multi').data('html');
        parent.find('.render .from_hour .multi').html(fromHtml.replace(':fromHourData', from_hour));

        let toHtml = parent.find('.render .to_hour .multi').data('html');
        parent.find('.render .to_hour .multi').html(toHtml.replace(':toHourData', to_hour));
    }
    ...
    $('.bravo-contact-block-form').submit(function (e) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (json) {
                window.location.replace(bookingCore.url + "/contact-thanks");
