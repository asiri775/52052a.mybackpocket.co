...
class SpaceController extends Controller
{
    ...
    public function searchView(): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View|\Illuminate\Contracts\Foundation\Application
    {
        ...
        if (($request->isMethod('post'))) {
            if (($request->has("title"))) {
                ...
                foreach ($space->terms as $t) {
                    if ($t->term->attr_id == 3) $cats[] = $t->term->name;
                    if ($t->term->attr_id == 4) $amns[] = $t->term->name;
                }
                shuffle($amns);
                if (count($amns) > 3) $amns = array_slice($amns, 0, 3);


                $yourApiKey = config('apiKey.api_key');
