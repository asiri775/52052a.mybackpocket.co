...
class BookingController extends AdminController
{
    ...
    public function sendNotificationEmail(Request $request, $id)
    {
        $booking = Booking::find($id); 
        // $customer = User::where('id', $booking->customer_id)->first();
        // Mail::to($customer->email)->send(new NewBookingEmail($booking, 'customer'));
        $booking->sendNewBookingEmails('customer');
        return redirect()->back()->with('success', __('Notification Email sent'));
    }
}
