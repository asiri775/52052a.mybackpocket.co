...
class Booking extends BaseModel
{
    ...
    public function sendNewBookingEmails($to = 'all')
    { 
        try {
            ...
            // to Vendor
            if ($to === 'all' || $to === 'vendor') {
                $EmailSubject = EmailSubject::where('token', 'g31nl9m')->first();
            ...
            if ($to === 'all' || $to === 'customer') {
                $EmailSubject_customer = EmailSubject::where('token', 'x33nl7b')->first();
                ...
                // Mail::to('kasyap459@gmail.com')->send(new NewBookingEmail($this, 'customer', $EmailSubject_customer['subject'], $EmailTemplate_customer));
            }
    ...
    public function sendStatusUpdatedEmails()
    {
        ...
        try {
            ...
            // to Vendor
            $EmailSubject = EmailSubject::where('token', 'b32nl8c')->first();
            $EmailTemplate = EmailTemplate::where('domain', 9)->where('subject_id', $EmailSubject['id'])->first();
            Mail::to(User::find($this->vendor_id))->send(new StatusUpdatedEmail($this, 'vendor', $EmailSubject['subject'], $EmailTemplate));

            // To Customer
            $EmailSubject_customer = EmailSubject::where('token', 'h32gl5c')->first();
            $EmailTemplate_customer = EmailTemplate::where('domain', 9)->where('subject_id', $EmailSubject_customer['id'])->first();
