...
class User extends Authenticatable implements MustVerifyEmail, JWTSubject, Wallet
{
    ...
    public function getAvatarUrl()
    {
        ...
        if ($this->avatar == null) {
            ...
            $this->avatar = "images/avatars/avatar-" . $randomAvatarKey . "-min.png";
            $this->save();
        }
        return $this->avatar;
    ...
    {
        $EmailSubject = EmailSubject::where('token', 'n31mk623')->first();
    ...
    public function sendEmailVerificationNotification()
    {
        ...
        $EmailSubject = EmailSubject::where('token', 'l5jhf24')->first();
        $EmailTemplate = EmailTemplate::where('domain', 9)->where('subject_id', $EmailSubject['id'])->first();

        $EmailSubject = setting_item('subject_email_verify_register_user');
        // $EmailTemplate = [];
        //$EmailTemplate['content'] = setting_item('content_email_verify_register_user');
